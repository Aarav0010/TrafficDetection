...
int main() {
    try {
        ...
        YOLOModel carModel(env, L"yolov8n.onnx", 80);     // COCO: 80 classes

        std::cout << "[INFO] Loading sign model (best.onnx)..." << std::endl;
        YOLOModel signModel(env, L"best.onnx", 8);          // 8 sign classes

        std::cout << "[INFO] Loading speed model (best (3).onnx)..." << std::endl;
        YOLOModel speedModel(env, L"best (3).onnx", 14, 960);    // 14 speed limit classes, 960x960 input
